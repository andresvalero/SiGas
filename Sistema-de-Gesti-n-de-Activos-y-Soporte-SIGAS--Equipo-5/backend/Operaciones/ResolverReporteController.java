...
@WebServlet("/ResolverReporte")
public class ResolverReporteController extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            PreparedStatement ps1 = conn.prepareStatement(sqlReporte);
            ps1.setInt(1, id_reporte);
            ps1.executeUpdate();
            ps1.close();

            // 2. Regresar el equipo a estado Disponible en el inventario
            ...
            conn.close();
            response.sendRedirect(request.getContextPath() + "/Frontend/reportes/reportes.jsp?resolucion=exito");
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(request.getContextPath() + "/Frontend/reportes/reportes.jsp?error=bd");
        }
    }
}
